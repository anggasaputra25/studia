...
export default function QuizList({courseId}: {courseId: string}) {
    ...
    if(error && error.length > 0) {
        return <div className={'w-full border-2 border-gray-500/20 rounded-xl flex justify-between items-center p-3 mt-5'}>
    ...
    return (
        <div>
            {quizzes.map((quiz) => (
                    <div key={quiz.id}
                         className={'w-full border-2 border-gray-500/20 rounded-xl flex justify-between p-3 mt-5'}>
                        <div className={'flex gap-4'}>
                            ...
                            <div className={'flex flex-col justify-center'}>
                                <Link href={`/courses/${courseId}?tab=quiz&q=${quiz.id}`} passHref={true} key={quiz.id} className={'cursor-pointer'}>
                        ...
                        <div className={'flex gap-2 items-center'}>
                            <Button
                                ...
                                size={'sm'} className={`${!((!quiz.attended && !quiz.finished ) || (quiz.attended && !quiz.finished)) || "drop-shadow-[0px_4px_0px#FFAE00] bg-[#FFCB67] font-medium hover:bg-[#FFCB67]"}`}>
