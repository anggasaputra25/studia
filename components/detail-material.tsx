                ...
                {materials.map((material, index) => (
                    <div className="p-5 border rounded" key={index}>
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-lg">{`${course?.instructor} menambahkan materi pada > ${course?.name}`}</p>
                                <p className="text-muted-foreground text-sm">{course?.created_at ? timeAgo(course.created_at) : ""}</p>
                            </div>
                        </div>
                        <div className="mt-5">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    {material.material_files.length > 0 ? (
                                        material.material_files.map((file, i) => (
                                            <Link className="text-muted-foreground text-sm" href={file.file_url} key={i}>{file.file_name}</Link>
                                        ))
                                    ) : (
