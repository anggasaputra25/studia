...
export function DoQuiz() {
    ...
    const [answerOptions, setAnswerOptions] = useState<AnswerOption[] | null>(null);
        ...
        async function fetchQuestion() {
            try {
                ...
                const {data, error: questionError} = await supabase
                    .from('quiz_questions')
                    .select('*, quiz_answer(id, qq_id, student_id, option, student_answer)')
                    .eq('quiz_id', quizId)
                    .eq('number', number)
                    .single();
            ...
            try {
                ...
                const {error} = await supabase
                    ...
                    .upsert({
                        ...
                        is_correct: studentAnswer.student_answer === question?.correct_answer
                    }, {
                        onConflict: 'qq_id'
                    })
    ...
    const handleAnswering = (option: AnswerOption) => { // Remove async here
    ...
    const handleFinish = async () => {
        try {
            ...
            toast.success('Hore! Kamu sudah menyelesaikan quiz ini');
    ...
    return (
        <div className={'mt-5'}>
            {quiz && (
                <div className="border-2 flex  items-center gap-4 border-gray-500/20 rounded-xl p-4">
                    ...
                    <div className={'flex items-center justify-between mt-5 gap-2'}>
                        ...
                        {!answerOptions ?
                            ...
                            answerOptions.map((option, index: number) => {
                                const backround = option.answer === studentAnswer.student_answer ? 'bg-green-500/5 border-2 border-green-500/30' : 'bg-gray-500/10 border-2 border-gray-500/20';
                                return (
                                    <div
            ...
            {!questionLoading && (
                <div className={'mt-5 flex justify-end'}>
                    <div className="flex justify-between mt-5 gap-2">
                        <Button
                            onClick={() => router.replace(`?tab=quiz&q=${quizId}&n=${number - 1}&attempt=true`)}
                            disabled={number <= 1}
