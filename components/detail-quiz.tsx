  ...
  return (
    <div className="space-y-6 mt-5">
      {quiz && (
        <div
          className={
            "border flex  items-center gap-4 justify-between border-gray-500/20 rounded-xl p-4"
          }
        >
          ...
          <AlertDialog>
            <AlertDialogTrigger>
              <div
                className={
                  "flex p-3 rounded-full cursor-pointer bg-red-500/5 border border-red-500/50 gap-2 items-center"
                }
              >
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Apakah kamu yakin ingin menghapus kuis ini? Aksi ini tidak
                  dapat dibatalkan.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
      ...
      {quiz && !quiz.attended && !quiz.finished && (
        <div className={"flex flex-col items-center justify-center mt-5"}>
          ...
          <p className={"text-sm text-gray-300 mt-2"}>
            Ayo kerjakan sekarang dan lihat hasilnya
          </p>
          <Button
            onClick={() =>
              router.replace(`?tab=quiz&q=${quiz.id}&attempt=true`)
            }
            className={
              "bg-[#FFCB67] font-semibold hover:bg-primary-darker drop-shadow-[0px_4px_0px#FFAE00] mt-5"
            }
          >
            Kerjakan sekarang
          </Button>
      ...
      {quiz && quiz.attended && quiz.finished && (
        <div className={"mt-5"}>
          <div className={"p-5 border-gray-500/20 rounded-xl"}>
            ...
            <p className={"text-sm text-gray-400"}>
              Selesai pada{" "}
              {quiz.quiz_result?.created_at
                ? new Date(quiz.quiz_result.created_at).toLocaleString(
                    "id-ID",
                    {
                      dateStyle: "medium",
                      timeStyle: "short",
              ...
              return (
                <div key={index} className={"mt-5 p-5 rounded-xl"}>
                  ...
                  {options.map((option, index: number) => {
                    const background =
                      question.quiz_answer?.option === option.option
                        ? question.quiz_answer.is_correct
                          ? "bg-green-500/5 border border-green-500/20"
                          : "bg-red-500/5 border border-red-500/20"
                        : "bg-gray-500/5 border border-card-border/10";
                    return (
                      <div
                        key={index}
                        className={`p-5 ${background} rounded-xl mt-3`}
                      >
                        <h1 className={"text-md font-bold"}>{option.option}</h1>
                        <p className={"text-md"}>{option.answer}</p>
                      </div>
                    );
                  })}
                </div>
