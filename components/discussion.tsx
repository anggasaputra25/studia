"use client";
import type {
  AdditionalDiscussionFiles,
  Discussion,
  DiscussionFile,
  DiscussionFiles,
  Material,
  MaterialFiles,
} from "@/app/types/discussion";
...
const Discussion = () => {
  ...
  const [discussionDataFile, setDiscussionDataFile] = useState<
    DiscussionFiles[]
  >([]);
  const [additionalDiscussionDataFile, setAdditionalDiscussionDataFile] =
    useState<AdditionalDiscussionFiles[]>([]);
  ...
  const regenerateAnswer = async (prompt: string, discussion_id: string) => {
    ...
    try {
      const res = await fetch(
        `/api/courses/${course_id}/discussion/${discussion_id}/regenerate`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const editAnswer = async (discussion_id: string) => {
    ...
    try {
      const res = await fetch(
        `/api/courses/${course_id}/discussion/${discussion_id}/update`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="min-h-screen bg-[#171717] flex flex-col text-white">
                ...
                <div key={item.id} className="space-y-6">
                      ...
                      disFile.discussion_id == item.id
                        ? materialFiles.map((matFile) =>
                            disFile.mf_id == matFile.id ? (
                              <a
                                href={matFile.file_url}
                                key={matFile.id}
                                target="_blank"
                                className="justify-end mt-0 flex items-center text-sm text-gray-400"
                              >
                                <div className="flex items-center bg-[#1a1a1a] rounded-lg px-3 py-2 border card-border hover:bg-primary hover:text-white transition-all ease">
                                  <Paperclip className="w-4 h-4" />
                                  <span>{matFile.file_name}</span>
                                </div>
                              </a>
                            ) : (
                              ""
                  ...
                  <div className="flex items-center space-x-2 justify-end">
                    ...
                    <AlertDialog>
                      <AlertDialogTrigger
                        className={`p-2 hover:bg-[#2a2a2a] rounded-lg transition-colors group ${
                          loadingRegenerateAnswer ? "cursor-not-allowed" : ""
                        }`}
                        onClick={() => setInputPromptNew(item.prompt)}
                        disabled={loadingUpdatePrompt}
                      >
                        <Pencil className="w-4 h-4 text-gray-500 group-hover:text-gray-300" />
                      </AlertDialogTrigger>
                      <AlertDialogContent>
                        <AlertDialogHeader>
                  ...
                  <div className="max-w-none">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        ...
                        disabled={loadingRegenerateAnswer}
                        onClick={() => regenerateAnswer(item.prompt, item.id)}
                      >
      ...
      {loadingSendMessage && (
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center gap-3">
            <div className="bg-[#1a1a1a] rounded-xl px-4 py-3 border card-border">
      ...
      {/* Input Area */}
      <div className="bg-[#1a1a1a] sticky bottom-1 rounded-lg border card-border">
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="relative">
            ...
            <div className="flex items-start space-x-3 rounded-xl p-3 border-none transition-colors pb-0">
              ...
              {/* Send Button */}
              <button
                className={`flex-shrink-0 p-2 rounded-lg m-0 transition-all ${
                  !inputPrompt.trim() || loadingSendMessage
                    ? "text-gray-500 cursor-not-allowed border border-gray-800"
                    : "bg-primary text-background hover:bg-primary-darker"
                }`}
            ...
            <div className="mb-4 flex">
              ...
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button
                    variant="outline"
                    className="flex items-center space-x-2 text-gray-400 hover:text-gray-300 text-sm p-2 mt-2 hover:bg-[#1a1a1a] rounded-lg transition-colors"
                  >
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogTitle>
                      Pilih file materi dari kelas
                    </AlertDialogTitle>
                    <div className="flex gap-3 flex-wrap">
                      {materialData.map((item) =>
                        materialFiles
                          .filter(
                            (fileItem) => fileItem.material_id === item.id
                          )
                          .map((fileItem) => {
                            const isSelected = file.some(
                              (f) =>
                                f.id === fileItem.id &&
                            ...
                            return (
                              <button
                                ...
                                className={`border p-2 rounded-sm transition ${
                                  isSelected
                  ...
                  <AlertDialogFooter>
                    ...
                    <AlertDialogAction className="bg-yellow-400">
                      Oke
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
                    ...
                    {[...file, ...additionalFile].length > 1 && (
                      <div className="mt-2 flex items-center space-x-2 text-sm text-gray-400">
                        <div className="flex items-center space-x-2 bg-[#1a1a1a] rounded-lg px-3 py-2">
                          <Paperclip className="w-4 h-4" />
                          <span>
                            {[...file, ...additionalFile].length - 1} file
                            lainnya
                          </span>
                        </div>
                      </div>
