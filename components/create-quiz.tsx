...
export default function CreateQuiz({ courseId }: { courseId: string }) {
  ...
  const [selectedMaterialFiles, setSelectedMaterialFiles] = useState<
    Map<number, string>
  >(new Map());
    ...
    <div>
      <div className={"flex gap-5 mt-5"}>
        ...
        <div className={"flex flex-col justify-center"}>
          <h2 className={"text-lg font-bold"}>Buat kuis</h2>
          <p className={"text-sm text-gray-400"}>
            Pilih materi dan AI akan membuat kuis secara otomatis.
          </p>
        </div>
      </div>
        ...
        {materialLoading ? (
          ...
          <div className={"flex gap-3 mt-5"}>
            {materialFiles.map((materialFile, index: number) => {
              const backround = !selectedMaterialFiles.has(index)
                ? "bg-gray-500/10 border border-card-border/10 rounded-full px-5 py-1"
                : "bg-gray-500/30 border border-card-border/10 rounded-full px-5 py-1";
              return (
                <div
                  onClick={() =>
                    handleSelectMaterialFile(index, materialFile.file_url)
                  }
                  key={materialFile.id}
                  className={`cursor-pointer text-[#A1A1A1] ${backround}`}
                >
      ...
      <div className={"mt-5"}>
        <h3 className={"font-bold text-md"}>Upload file</h3>
        <p className={"text-sm text-gray-400"}>
        ...
        <div className={"mt-5"}>
          ...
          <div
            ...
            className={
              "p-5 border-dashed border cursor-pointer bg-gray-500/5 border-gray-500/50 rounded-xl w-full h-64 flex flex-col justify-center items-center text-gray-400"
            }
          >
            <div
              className={
                "bg-gray-500/10 border border-gray-500/30 rounded-xl p-8"
              }
            >
              <File size={40} />
            </div>
          ...
          return (
            <div key={index} className={"py-2 flex justify-between"}>
              <div className={"flex gap-3 items-center"}>
                <div
                  className={
                    "p-3 rounded-xl bg-gray-500/10 border border-card-border/10 w-15 h-15"
                  }
                >
                  <File />
                </div>
        ...
        <AlertDialog open={generateLoading} onOpenChange={setGenerateLoading}>
          <AlertDialogTrigger asChild>
            <Button
              ...
              disabled={
                (selectedMaterialFiles.size === 0 &&
                  uploadedFiles.length === 0) ||
                generateLoading
              }
              className={
                "bg-[#FFCB67] font-medium hover:bg-primary-darker drop-shadow-[0px_4px_0px#FFAE00]"
              }
            >
          ...
          <AlertDialogContent>
            ...
            >
              <div
                className={
                  "w-1/2 h-3/4 p-10 bg-gray-500/10 border-gray-500 rounded-xl flex justify-center items-center mx-auto"
                }
              >
                <GeneratingQuizIcon />
              </div>
              ...
              <AlertDialogDescription className={"text-center"}>
                Mohon tunggu sebentar, kuis sedang dalam proses pembuatan. Ini
